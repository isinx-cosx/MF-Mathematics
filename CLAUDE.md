...
- 项目根目录下必须有一个 `.gitignore` 文件，排除 `__pycache__`、`*.pyc`、`.vscode`、`logs/`、`temp/`、`*.log` 等临时文件。如果当前没有，请先创建。
- **每次修改代码前**：AI 必须执行 `git add -A && git commit -m "自动保存：修改前状态 [时间]"` 来保存当前工作状态（无需运行测试，因为这是已知的稳定状态）。
- **修改完成后**：AI 必须先尝试运行 `python main.py`。如果程序能够正常启动且控制台无报错，则执行 `git add -A && git commit -m "自动保存：修改后状态 - [简述改动]"` 保存修改结果。
- **如果启动失败**（报错或退出）：AI 应回退到上一个提交点（`git reset --hard HEAD`），并提示“本次修改导致程序无法启动，已撤销”。
- 提交信息使用中文，简述改动内容（如“修复步骤查看器缓存问题”）。
- AI 不需要执行 `git push`，只负责本地提交。
- 若遇到冲突或需要回退，可随时使用 `git log` 查看保存点，或手动执行 `git reset --hard <commit_hash>`。
