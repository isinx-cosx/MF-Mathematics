...
def build():
    ...
    add_img(doc, "10_calc_algebra.png", 5.5)
    for title, desc in [
        ("4.1 基础运算", "四则运算、乘方、开方、阶乘、对数等基本数学运算。支持表达式连续输入和即时求值。"),
        ("4.2 代数计算", "多项式展开、因式分解、方程求解、不等式求解、表达式化简。支持多计算块并行工作。"),
    ...
    tip(doc, "代数计算等多数模块支持多块管理——点击\"添加计算块\"可并行进行多个独立计算。基础运算为单块即时求值模式。")
    ...
    add_img(doc, "08_ai_dialog.png", 4.5)
    ...
    add_img(doc, "07_keyboard.png", 5.5)
    ...
    add_img(doc, "09_settings.png", 4.5)
    doc.add_heading("9.3 关于", level=2)
    doc.add_paragraph("菜单 → 帮助 → 关于，查看版本信息和开发团队。")
    add_img(doc, "06_about.png", 4.0)
