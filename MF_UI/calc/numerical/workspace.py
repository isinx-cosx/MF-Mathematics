# ————————————MF-Mathematics 数值分析工作区 ————————————

# 导入必要库以及自定义组件
import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
from calc.numerical.calc_block import CalcBlock
from PySide6.QtWidgets import *

# 定义 Workspace 类 —— 数值分析工作区 ——
class Workspace(QWidget):
    def __init__(self):
        super().__init__()

        # 初始化标题和描述标签
        title_label = QLabel("数值分析")
        title_label.setObjectName("title_label")
        desc_label = QLabel("包含：插值、数值积分、ODE 数值解、非线性方程求根等")
