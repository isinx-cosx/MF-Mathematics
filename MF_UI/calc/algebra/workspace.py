# ————————————MF-Mathematics 代数计算工作区 ————————————

# 导入必要库以及自定义组件
import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
from calc.algebra.calc_block import CalcBlock
from PySide6.QtWidgets import *

# 定义 Workspace 类 —— 代数计算工作区 ——
class Workspace(QWidget):
    def __init__(self):
        super().__init__()

        # 初始化标题和描述标签
        title_label = QLabel("代数计算")
        title_label.setObjectName("title_label")
        desc_label = QLabel("包含：表达式化简、求导、积分、极限、级数展开、复数运算等")
